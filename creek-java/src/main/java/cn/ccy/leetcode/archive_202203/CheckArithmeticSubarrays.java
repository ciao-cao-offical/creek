...
public class CheckArithmeticSubarrays {
    ...
    public List<Boolean> checkArithmeticSubarrays(int[] nums, int[] l, int[] r) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            int[] sub = Arrays.copyOfRange(nums, startIndex, endIndex + 1);
            Arrays.sort(sub);
            //差值
            int d = sub[1] - sub[0];
            int subLen = sub.length;
            boolean flag = true;
            for (int j = 2; j < subLen; j++) {
                if (sub[j] - sub[j - 1] != d) {
                    flag = false;
                    break;
                }
